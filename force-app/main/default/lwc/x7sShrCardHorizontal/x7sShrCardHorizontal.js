...
export default class x7sShrCardHorizontal extends NavigationMixin(LightningElement) {
	...
	@api imageWidth = ''; // Forces a specific width to be set horizontally. Otherwise, uses styling. Vertical Image Width is always 100%.
	@api iconOpacity = ".3";
	@api iconColor = "";
	...
	// If there is no image background then don't use the image-width-horizontal and calculate the correct width
	get calculatedImageWidth() {
	...
	get componentClass() {
		return classSet(`card-horizontal ${this.customClass} `)
			.add({
	...
	get componentStyle() {
		return this.componentHeight !== '' ? `min-height: ${this.componentHeight};` : '';
	}
	...
	get componentHeight() {
		...
		const mheight = getInteger(this.minHeight);
		let aheight = getInteger(this.avatarSize);
		if (this.variant === 'featured') {
			// Add 30 pixels because we want SOME padding around the avatar if it's in featured.
			aheight = aheight + 30;
		}

		// If min-height is less than the avatar height, use the avatar height
		return `${mheight < aheight ? aheight : mheight}px`;
	}
    ...
    get imageStyle() {
		let imageWrapperStyle = this.calculatedImageWidth !== '' ? `flex-basis: ${this.calculatedImageWidth};` : '';
