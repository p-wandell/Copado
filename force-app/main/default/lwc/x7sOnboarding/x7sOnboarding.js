...
/* ---- Builder Properties ---- */
    ...
    get isPage4() {
        return ( this.displayTopics || this.page === '4' );
    }
    ...
    getUserRecord() {
        ...
        currentUserRecord()
            .then(result => {
                ...
                if(result) {
                    ...
                    if(this.isModalViewEnabled){
                        if( (this.userModalDisplayCount <= this.metaDisplayLimit) && this.closeModalByUser === false && 
                        (this.metaDisplayException === undefined || this.metaDisplayException.includes(this.profileName)) ){
                            this.displayOnboardingSection = true;
                            this.displayLimit();
                            console.log("----enter in----open modal---");
                            return; 
                        }
            ...
            .catch(err => {
                this.error = err;
                console.log('getUserRecord Failed:' + err);
            });
                ...
                if(stepList.length > 0) {
                    for(currentStep = 0; currentStep < stepList.length; currentStep++){
                        ...
                        completeSlide({slide:stepList[currentStep]})
                            .then((result) => {
                                console.log('success:excludeSteps:' + completedSlide);
                            })
                            .catch((err) => {
                                this.error = err;
                                console.log('excludeSteps Failed:' + err);
                            });
                    }
                }
            }
            ...
            if(this.closeModalByUser !== true){
                ...
                if(this.metaDisplayLimit === 0 || this.metaDisplayLimit === undefined) {
                    ...
                    if(this.userModalDisplayCount < this.metaDisplayLimit){
                        if(this.metaDisplayException === undefined || !this.metaDisplayException.includes(profileName)){
                            this.userModalDisplayCount += 1;
                            console.log('====After Increment===='+this.userModalDisplayCount);
                            this.displayLimit();
                            this.openSlide(event); 
                        }
                        else if(this.metaDisplayException.includes(profileName)){
                            this.openSlide(event);
                        } 
                    }
                }
...
console.log('handleNextStepChange:pageNumber ==> ', pageNumber);
    ...
    topicSlideRetrieval(){
        ...
        if(useMetadata) {
            ...
            grabTopics()
                ...
                .catch((err) => {
                    this.error = err;
                    console.log('topicSlideRetrieval Failed:' + err);
                });
    ...
    retrieveIsModalViewEnabled() {
        ...
        IsModalViewEnabled()
            .then((result) => {
                ...
                console.log('success:IsModalViewEnabled '+this.isModalViewEnabled);
            })
            .catch((err) => {
                this.error = err;
                console.log('IsModalViewEnabled Failed:' + err);
            });
    }

    setUserModalDisplayCount() {

        console.log('-----entered----- Counter set to 0---');

        completeSlide({slide:'setDisplay'})
                .then((result) => {
                    this.page = pageNumber;
                    console.log('success:ModalDisplayCount:' + completedSlide);
                })
                .catch((err) => {
                    this.error = err;
                    console.log('ModalDisplayCount Failed:' + err);
                });
    }
    ...
    displayLimit() {
        ...
        completeSlide({slide:'displayLimit'})
                ...
                .catch((err) => {
                    this.error = err;
                    console.log('displayLimit Failed:' + err);
                });
    }
