...
({
	...
	getPicklists: function (component) {
		if (component.get('v.showPicklists')) {
			...
			for (let pos = 1; pos <= this.custom.MAX_FIELDS; pos++) {
				let customField = component.get('v.customField' + pos);

				if (customField.length) {
					let auraId = 'typeAheadCustomField' + pos;
	...
	getMemberPicklistValues : function (component, fieldAuraId, fieldName, valueSet, searchString) {
		const action = component.get('c.getMemberPicklist');
	...
	getTopicValues : function(component) {
		const action = component.get('c.getTopicValues');
		action.setStorable();

		action.setCallback(this, function (actionResult) {
			if (actionResult.getState() === 'SUCCESS') {
				const results = actionResult.getReturnValue();
				let values  = [];
	...
	fireSearchEvent: function (component, clearAll)
	{
		...
		let myMembers    = component.get('v.showMyMembers');
		let topicString  = '';
		...
		if (component.get('v.showTitleFilter')) {
			let titleSearch = component.get('v.titleSearch');
		...
		if (component.get('v.showCountryFilter')) {
			let countrySearch = component.get('v.countrySearch');
		...
		if (component.get('v.showStateFilter')) {
			let stateSearch = component.get('v.stateSearch');
		...
		if (component.get('v.showCityFilter')) {
			let citySearch = component.get('v.citySearch');
		...
		if (component.get('v.showAccountFilter')) {
			let accountSearch = component.get('v.accountSearch');
		...
		if (component.get('v.showTopicFilter')) {
			let topicSearch = component.get('v.topicSearch');
		...
		for (let pos = 1; pos <= this.custom.MAX_FIELDS; pos++) {
			...
			let customField = component.get('v.customField'+pos);
			if (customField.length) {
				let customSearch = component.get('v.searchCustomField'+pos);
		...
		// the empty search = '::;'
		component.set('v.disableClear', searchString.length <= 3 && topicString === '' && !myMembers);

		const searchEvent  = $A.get("e.c:SVNSUMMITS_Members_Filters_Event");
	...
	getTranslatedText: function (component, source, destination, labelHandle) {
		// const labelRegex = /^\$Label\.([a-zA-Z0-9_]*\.){1}([a-zA-Z0-9_]+)$/;
		const labelRegex = /^\$Label\.([a-zA-Z0-9_]*\.)([a-zA-Z0-9_]+)$/;
		//console.log("source = ", source);
		let text = component.get(source);
